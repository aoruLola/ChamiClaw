...
### 3) 关键现状问题
- `run-once` 在实跑中出现过卡住（外层执行曾被 SIGKILL），稳定性需优先排查。
- 当前统计仍出现 `signals=0`、`orders=0`，说明信号触发与执行链路未形成有效产出。
- 结论：目前适合“数据与研究验证”，不适合直接实盘。

### 4) 最新优先级（按阻塞程度）
1. **真实 LLM 接入替代 mock**（并保证失败降级）
2. **真实执行闭环**（下单/查单/撤单/状态归一）
3. **真实风险与对账输入**（去掉占位值，接真实仓位与成交）
